...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen3 Interactive CLI")
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Enable Qwen3 thinking mode (default: on, use --no_thinking to disable)"
    )
...
def load_model_and_tokenizer(model_path, device):
    ...
    print(f"{Color.GREEN}Model loaded! Parameters: {param_count:.2f}B{Color.RESET}")
...
def format_messages(messages, tokenizer, enable_thinking=False):
    ...
    if hasattr(tokenizer, 'chat_template') and tokenizer.chat_template is not None:
        try:
            # Qwen3 的推荐方式：通过 chat_template_kwargs 控制 thinking
            ...
            # 某些版本的 transformers 不支持 chat_template_kwargs
            try:
                return tokenizer.apply_chat_template(
                    messages,
                    tokenize=False,
...
def chat_loop(model, tokenizer, args, device):
    ...
    while True:
        try:
            ...
            # 只有 thinking 关闭时，才用  作为 eos（防止无限 think）
            # thinking 开启时，模型需要先 think 完再回答，不能中途停
            if not args.enable_thinking:
                if think_end_token_id is not None:
                    eos_ids.add(think_end_token_id)
            eos_ids = list(eos_ids)
            
            # ── 构建 StoppingCriteria（兜底） ──
            if not args.enable_thinking:
