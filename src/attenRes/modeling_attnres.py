...
class Qwen3AttnResModel(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Run the Qwen3AttnRes decoder stack.

        Args:
            input_ids: Token ids for the input sequence.
            attention_mask: Attention mask or precomputed causal mask mapping.
            position_ids: Position indices used for rotary embeddings.
            past_key_values: Cache instance carrying previously computed key/value states.
            inputs_embeds: Precomputed input embeddings. Provide this instead of `input_ids`.
            use_cache: Whether to update and return the KV cache.
            cache_position: Absolute token positions used when reading from or writing to the KV cache.
            **kwargs: Additional keyword arguments forwarded through the model stack.
        """
...
class Qwen3AttnResForCausalLM(Qwen3PreTrainedModel, GenerationMixin): # GenerationMixin如果你不给模型加上这个“混入类”，你的模型就只能像个算盘一样，拨一下算一下（只能根据输入预测下一个词）；加上它之后，你的模型就变成了一个打字机，能够自动、连续地吐出完整的文章。
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Run causal language modeling on top of the Qwen3AttnRes backbone.
        ...
        Args:
            ...
            past_key_values: Cache instance carrying previously computed key/value states.
            inputs_embeds: Precomputed input embeddings. Provide this instead of `input_ids`.
            labels: Target token ids used to compute the language modeling loss.
            use_cache: Whether to update and return the KV cache.
            cache_position: Absolute token positions used when reading from or writing to the KV cache.
            logits_to_keep: Slice spec controlling which logits are projected by the LM head.
            **kwargs: Additional keyword arguments forwarded to the backbone and loss function.
        """
