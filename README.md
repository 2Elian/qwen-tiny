...
- [OPD训练](/docs/continue_pretrained_attnRes.md)在这里我们使用rl出来的医疗领域模型+nl2sql模型(均来自于base模型冷启动+rl后的检查点)，两个专家模型来在线蒸馏base模型，让base模型拥有二者能力。并且我们将会做AB实验，即将两个专家模型合并起来(两个数据源训一个模型)，对比蒸馏效果。
